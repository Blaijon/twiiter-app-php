		...
		public function notification($user_id){
			$stmt = $this->pdo->prepare("
				SELECT * FROM `notification` N 
						...
						LEFT JOIN `follow` F ON N.`notificationFrom` = F.`sender` AND N.`notificationFor` = F.`receiver` WHERE N.`notificationFor` = :user_id AND N.`notificationFrom` != :user_id");
			$stmt->execute(array("user_id" => $user_id));
			return $stmt->fetchAll(PDO::FETCH_OBJ);
		}
		public function sendNotification($get_id, $user_id, $target,$type) {
			$this->create('notification', array('notification' => $get_id, 'notificationFrom'=> $user_id, 'target' =>$target, 'type' => $type, 'time'=> date('Y-m-d H:i:s')));
		}
	}
